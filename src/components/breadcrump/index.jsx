    ...
    return (
        <div className="w-full h-[150px] sm:flex hidden overflow-x-hidden  relative z-0 ">
            ...
            <div className="absolute left-12 w-1/4  top-[40%] overflow-x-hidden  text-white flex justify-center items-center z-10">
                <p className="text-2xl font-bold text-slate-100 dark:text-white uppercase text-xl">{title}</p>
            </div>
            <div className="absolute right-12 overflow-x-hidden bottom-1 flex justify-center items-center z-20">
                {path.map((pat, i) => (<Link key={i} to={`/${pat.path}`} className={` capitalize ${pat.path === pathname ? "text-slate-300  underline  " : "text-slate-100 dark:text-orange-600 cursor-pointer"} text-sm px-1`}>{pat.title}{pat.path !== pathname && "|"}</Link>))}
            </div>
